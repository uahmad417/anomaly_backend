...
def source_ips():
    print(scapy_index)
    response = es.search(index=eval(scapy_index), body = queries.query_source_ips)
...
def web_activity(ip):
    ...
    timeline_json = []
    response = es.search(index=eval(scapy_index), body=queries.query_webpages%ip, scroll='5m')
...
def command_injection(payload):
    sysmon_process_response = es.search(index=eval(sysmon_index),body=queries.query_command_injection%requests.utils.unquote(payload.replace('+','%20')))
    sysmon_event = {}
...
def chain(process_id):
    sysmon_process_response = es.search(index=eval(sysmon_index),body=queries.query_child_pids%process_id)
...
def store_process(current_process_id):
    sysmon_process_response = es.search(index=eval(sysmon_index),body=queries.query_pid%current_process_id)
    sysmon_event = {}
    for entry in sysmon_process_response['hits']['hits'][0]['_source']['Event']['EventData']['Data']:
        sysmon_event.update(entry)
    anomaly_json['process'].append(sysmon_event)
...
def file_creation(process_id):
    sysmon_file_response = es.search(index=eval(sysmon_index),body=queries.query_file_creation%process_id)
...
def network_connections(process_id):
    sysmon_network_response = es.search(index=eval(sysmon_index),body=queries.query_network_connect%process_id)
